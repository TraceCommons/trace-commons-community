...
A SwiftUI menu-bar app for macOS, a GTK 4/libadwaita window for Linux, and a WinUI 3 window for Windows. The Linux window connects to the systemd-managed contributor daemon; the macOS and Windows apps host the daemon in-process.

All three carry the full path — enrol, preview, approve, receipt — so everything below under *What the app can do* applies to each of them. The Windows app is the newest and is still missing pieces at the edges: bulk withdrawal (a refusal rather than a gap — `withdraw_bulk` returns only counts, and the withdrawal contract forbids reporting a generic "withdrawn", so withdraw per row instead), the watcher and connection sections of Settings, and most of the tray menu's vocabulary.
:::
