...
A successful upload returns a receipt with the submission identifier and an acceptance outcome. `accepted` and `quarantined` both mean the server received and stored the envelope; they differ in whether the trace can proceed without manual privacy review.
