...
Useful flags: `--dir <path>` to install elsewhere, `--version 0.2.0` to pin a release instead of taking the newest.
...
Or download a binary by hand from [contributor-v0.2.0](https://github.com/TraceCommons/trace-commons-server/releases/tag/contributor-v0.2.0), the current CLI release. Note the tag: this repository publishes CLI releases as `contributor-v*` and desktop-app releases as `app-v*`, so GitHub's "latest release" is whichever was cut most recently and may hold no CLI binaries at all. That is not hypothetical — the most recent release right now is `app-v0.2.1`, and it contains no CLI at all.
